...
function GoalDialog({ mode, goals, initialValue = "", initialGoalId, initialDomain, onCancel, onSave }: { mode: GoalDialogMode; goals: Goal[]; initialValue?: string; initialGoalId?: number; initialDomain?: string; onCancel: () => void; onSave: (data: GoalDialogSave) => void }) {
  ...
  const description = isPeriodEdit ? "Chỉ cập nhật trạng thái của tuần đang chọn." : mode === "period" ? "Thêm một mốc thời gian để theo dõi kết quả." : "Thông tin sẽ được hiển thị đồng thời ở Kế hoạch giáo dục và Tổng quan.";
  ...
  return <div className={`modal-backdrop ${isPeriodEdit || isEdit ? "small-edit-modal" : ""}`} role="presentation"><div className="modal-card goal-dialog" role="dialog" aria-modal="true" aria-labelledby="goal-dialog-title"><div className="modal-head"><div><h2 id="goal-dialog-title">{title}</h2><p>{description}</p></div><button type="button" className="close-button" onClick={onCancel} aria-label="Đóng">×</button></div><div className="form-grid">{mode === "long" && <SelectField label="Lĩnh vực áp dụng" value={domain} onChange={setDomain} options={[...new Set([...DOMAIN_OPTIONS, ...goals.map((goal) => goal.domain).filter(Boolean)])]} required />}{mode === "short" && <label className="field"><span>Mục tiêu dài hạn<em>*</em></span><div className="select-wrap"><select value={goalId} onChange={(event) => setGoalId(event.target.value)}>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.longTerm || "Chưa nhập mục tiêu"}</option>)}</select><Icon name="chevron" size={18} /></div></label>}{isPeriodEdit && <label className="field full"><span>Trạng thái kết quả<em>*</em></span><div className="select-wrap"><select value={status} onChange={(event) => setStatus(event.target.value as Status)}>{STATUS_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select><Icon name="chevron" size={18} /></div></label>}{mode !== "short" && !isPeriodEdit && <label className="field full"><span>{label}<em>*</em></span>{mode === "period" || mode === "domain" ? <input value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} autoFocus /> : <textarea value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} autoFocus />}</label>}{mode === "short" && <label className="field full"><span>{label}<em>*</em></span><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} autoFocus /></label>}{isEdit && !isPeriodEdit && <div className="field full"><small className="field-hint">Bạn đang cập nhật trực tiếp thông tin trong bảng mục tiêu.</small></div>}</div><div className="modal-actions"><button type="button" className="button" onClick={onCancel}>Hủy</button><button type="button" className="button primary" disabled={!isPeriodEdit && !text.trim()} onClick={() => onSave({ mode, text: isPeriodEdit ? status : text.trim(), domain, goalId: mode === "short" ? Number(goalId) : undefined })}><Icon name="save" size={17} />Lưu</button></div></div></div>;
}
