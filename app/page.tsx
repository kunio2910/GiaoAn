    ...
    {isOpen && <div className="plan-new-domain-body">
      {visibleGoals.length ? visibleGoals.map((goal, goalIndex) => <article className="plan-new-goal" key={goal.id}>
        ...
        <div><span className="plan-new-goal-label">Mục tiêu dài hạn {String(goalIndex + 1).padStart(2, "0")}</span><h4>{goal.longTerm || <span className="cell-placeholder">Chưa nhập mục tiêu dài hạn</span>}</h4></div>
          <div className="plan-new-goal-actions"><button className="plan-new-small-action" type="button" onClick={() => onEditLong(goal)}><Icon name="edit" size={15} />Sửa</button><button className="plan-new-small-action danger" type="button" onClick={() => onDeleteGoal(goal)} aria-label="Xóa mục tiêu dài hạn"><Icon name="trash" size={15} /></button></div>
        </div>
...
function PlanNewView({ childList, selectedChildId, onSelectChild, goals, hiddenDomains, evaluationPeriods, domainIcons, initialOpenDomains, onOpenDomainsChange, onSave, onAddDomain, onAddLong, onAddShort, onAddPeriod, onEditDomain, onEditLong, onDeleteGoal, onEditShort, onDeleteShort, onEditPeriod, onDeleteDomain, onStatusChange }: { childList: Child[]; selectedChildId: number; onSelectChild: (id: number) => void; goals: Goal[]; hiddenDomains: string[]; evaluationPeriods: string[]; domainIcons: Record<string, DomainIcon>; initialOpenDomains: Record<string, boolean>; onOpenDomainsChange: (openDomains: Record<string, boolean>) => void; onSave: (openDomains: Record<string, boolean>) => void | Promise<void>; onAddDomain: () => void; onAddLong: (domain?: string) => void; onAddShort: (goal: Goal) => void; onAddPeriod: () => void; onEditDomain: (goal: Goal) => void; onEditLong: (goal: Goal) => void; onDeleteGoal: (goal: Goal) => void; onEditShort: (goal: Goal, index: number) => void; onDeleteShort: (goal: Goal, index: number) => void; onEditPeriod: (goal: Goal, index: number) => void; onDeleteDomain: (domain: string) => void; onStatusChange: (id: number, periodIndex: number, status: Status) => void }) {
  const child = childList.find((item) => item.id === selectedChildId) ?? childList[0];
...
export default function Home() {
  ...
  const hideDomainInPlanNew = (domain: string) => {
    if (!window.confirm(`Ẩn lĩnh vực “${domain}” khỏi Kế hoạch giáo dục? Dữ liệu vẫn được giữ trong Cài đặt và Tổng quan.`)) return;
    setPlanNewHiddenDomainsByChild((items) => {
      const childKey = String(selectedChildId);
      const current = items[childKey] ?? [];
      return current.includes(domain) ? items : { ...items, [childKey]: [...current, domain] };
    });
  };
