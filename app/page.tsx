...
function shareExportHtml(child: Child, goals: Goal[], evaluationPeriods: string[], domainIcons: Record<string, DomainIcon>) {
  ...
  const rows = goals.length ? goals.map((goal) => `<tr><td><span class="export-domain-icon">${exportIconSvg(resolveDomainIcon(goal.domain, domainIcons))}</span><strong>${escapeExportHtml(goal.domain)}</strong></td><td>${escapeExportHtml(goal.longTerm || "Chưa nhập mục tiêu")}</td><td><ul>${(goal.shortTerm.length ? goal.shortTerm : ["Chưa có mục tiêu ngắn hạn"]).map((item) => `<li>${escapeExportHtml(item)}</li>`).join("")}</ul></td>${periods.map((_, index) => { const status = goal.statuses[index] ?? "Chưa đạt"; return `<td><span class="export-status ${statusClass(status)}"><i></i>${escapeExportHtml(status)}</span></td>`; }).join("")}<td>${escapeExportHtml(goal.note || "Chưa có ghi chú")}</td></tr>`).join("") : `<tr><td colspan="${periods.length + 4}" class="export-empty">Chưa có mục tiêu phát triển</td></tr>`;
