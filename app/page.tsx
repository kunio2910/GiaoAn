...
type View = "overview" | "plan" | "plan-new" | "children" | "settings" | "objective-form";
...
type AppCloudData = CloudData & {
  ...
  planNewOpenDomainsByChild?: Record<string, Record<string, boolean>>;
};
...
const PLAN_NEW_HIDDEN_DOMAINS_STORAGE_KEY = "giaoan-plan-new-hidden-domains";
...
function ChildChoiceButtons({ label, childList, selectedChildId, onSelectChild }: { label: string; childList: Child[]; selectedChildId: number; onSelectChild: (id: number) => void }) {
  return <div className="child-choice-field"><span className="field-label">{label}</span><div className="child-choice-buttons" role="group" aria-label={label}>{childList.map((child) => <button type="button" key={child.id} className={`child-choice-button ${child.id === selectedChildId ? "selected" : ""}`} onClick={() => onSelectChild(child.id)} aria-pressed={child.id === selectedChildId}><ChildAvatar name={child.name} /><span>{child.name}</span></button>)}</div></div>;
}
...
function exportIconSvg(name: DomainIcon) {
  const paths: Record<DomainIcon, string> = {
    overview: '<rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/>',
    plan: '<path d="M4 5.5A2.5 2.5 0 0 1 6.5 3H20v16H6.5A2.5 2.5 0 0 0 4 21.5z"/><path d="M4 5.5v16"/><path d="M8 7h8M8 11h8M8 15h5"/>',
    children: '<circle cx="9" cy="8" r="3"/><path d="M3.5 20a5.5 5.5 0 0 1 11 0"/><circle cx="17.5" cy="10" r="2.2"/><path d="M15.5 16.8a4.2 4.2 0 0 1 5 3.2"/>',
    target: '<circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="4"/><path d="m16.5 7.5 3-3M19.5 4.5v3h-3"/>',
  ...
  return `<svg class="export-domain-svg" style="width:23px;height:23px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round" viewBox="0 0 24 24" aria-hidden="true">${paths[name]}</svg>`;
}
...
function shareExportHtml(child: Child, goals: Goal[], evaluationPeriods: string[], domainIcons: Record<string, DomainIcon>) {
  ...
  const statusClass = (status: Status) => status === "Đạt" ? "status-achieved" : status === "Manh nha" ? "status-emerging" : status ? "status-not-achieved" : "status-empty";
  const goalGroups = goals.reduce<Array<{ domain: string; goals: Goal[] }>>((groups, goal) => { const group = groups.find((item) => item.domain === goal.domain); if (group) group.goals.push(goal); else groups.push({ domain: goal.domain, goals: [goal] }); return groups; }, []);
  const rows = goals.length ? goalGroups.map(({ domain, goals: domainGoals }) => domainGoals.map((goal, index) => `<tr>${index === 0 ? `<td class="export-domain-cell" rowspan="${domainGoals.length}"><span class="export-domain-icon">${exportIconSvg(resolveDomainIcon(domain, domainIcons))}</span><strong>${escapeExportHtml(domain)}</strong></td>` : ""}<td class="export-long-term-cell">${escapeExportHtml(goal.longTerm || "Chưa nhập mục tiêu")}</td><td class="export-short-term-cell"><ul>${(goal.shortTerm.length ? goal.shortTerm : ["Chưa có mục tiêu ngắn hạn"]).map((item) => `<li>${escapeExportHtml(item)}</li>`).join("")}</ul></td>${periods.map((_, periodIndex) => { const status = goal.statuses[periodIndex] ?? ""; return `<td class="export-result-cell"><span class="export-status ${statusClass(status)}"><i></i>${escapeExportHtml(status)}</span></td>`; }).join("")}<td class="export-note-cell">${escapeExportHtml(goal.note || "Chưa có ghi chú")}</td></tr>`).join("")).join("") : `<tr><td colspan="${periods.length + 4}" class="export-empty">Chưa có mục tiêu phát triển</td></tr>`;
...
function normalizeExportSummaryHtml(html: string, child: Child) {
  const summaryTable = `<table class="summary-table"><tbody><tr><td class="summary-main-cell"><strong>${escapeExportHtml(child.name)}</strong><span>Ngày sinh: ${escapeExportHtml(child.birthday)}</span><span>Tuổi thực: ${escapeExportHtml(calculateAge(child.birthday))}</span></td><td class="summary-meta-cell"><span>Người lập kế hoạch: Nguyễn Thị Vành Khuyên</span><span>Ngày lập kế hoạch: 30/06/2026</span></td><td class="evaluation-cell"><strong>Thông tin lượng giá</strong><span>Ngày lượng giá:</span><b>30/07/2026 và 30/08/2026</b></td></tr></tbody></table>`;
  const summaryStyles = `.summary-table{width:100%;table-layout:fixed;border-collapse:separate;border-spacing:0;background:#fff;border:1px solid #d7e4f4;border-radius:16px;padding:0;box-shadow:0 8px 22px rgba(42,83,148,.12)}.summary-table td{border:0;padding:18px 20px;vertical-align:middle;color:#10265f;line-height:1.5}.summary-table .summary-main-cell{width:38%;border-left:5px solid #1769d4;border-radius:16px 0 0 16px}.summary-table .summary-meta-cell{width:32%}.summary-table .evaluation-cell{width:30%;padding:14px;border:1px solid #bfe2d2;border-radius:12px;background:#f2fbf6;color:#126047}.summary-table .summary-main-cell strong{display:block;font-size:18px;color:#123f91;margin-bottom:10px}.summary-table .summary-main-cell span,.summary-table .summary-meta-cell span{display:block;margin-top:7px}.summary-table .evaluation-cell strong,.summary-table .evaluation-cell b{display:block;margin-bottom:7px}.export-domain-cell{width:15%!important;font-weight:700!important;vertical-align:middle!important;text-align:center}.export-long-term-cell{width:18%!important;font-weight:400!important}.export-short-term-cell{width:24%!important}.export-result-cell,.export-note-cell{font-weight:400!important}@media print{.summary-table{box-shadow:none}}`;
  return html.replace("</style>", `${summaryStyles}</style>`).replace(/<section class="summary">[\s\S]*?<\/section>/, summaryTable);
}
...
function openSharePrintWindow(child: Child, goals: Goal[], evaluationPeriods: string[], domainIcons: Record<string, DomainIcon>) {
  ...
  printWindow.document.write(normalizeExportSummaryHtml(shareExportHtml(child, goals, evaluationPeriods, domainIcons), child));
...
function downloadShareWord(child: Child, goals: Goal[], evaluationPeriods: string[], domainIcons: Record<string, DomainIcon>) {
  const blob = new Blob([`\ufeff${normalizeExportSummaryHtml(shareExportHtml(child, goals, evaluationPeriods, domainIcons), child)}`], { type: "application/msword" });
...
function StatusSelect({ value, onChange }: { value: Status; onChange: (value: Status) => void }) {
  return <label className={`status-select ${value === "Đạt" ? "achieved" : value === "Manh nha" ? "emerging" : value ? "not-achieved" : "pending"}`}>{value && <span className="status-dot" />}<select value={value} onChange={(event) => onChange(event.target.value as Status)} aria-label="Trạng thái kết quả">{STATUS_OPTIONS.map((option) => <option key={option || "empty"} value={option}>{option || "Chưa đánh giá"}</option>)}</select><Icon name="chevron" size={15} /></label>;
}
...
function ChildForm({ child, onCancel, onSave }: { child?: Child; onCancel: () => void; onSave: (child: Omit<Child, "id">) => void }) {
  ...
  const [color, setColor] = useState(child?.color ?? SCHEDULE_COLOR_OPTIONS[0].value);
...
function ChildrenView({ childList, onAdd, onEdit, onDelete, onSelectPlan }: { childList: Child[]; onAdd: () => void; onEdit: (child: Child) => void; onDelete: (id: number) => void; onSelectPlan: (id: number) => void }) {
  return <><Header title="Hồ sơ trẻ" subtitle={`${childList.length} hồ sơ đang được quản lý`} actionLabel="Thêm trẻ" onAction={onAdd} /><div className="children-intro"><div className="intro-icon"><Icon name="children" size={27} /></div><div><h2>Thông tin tất cả các trẻ</h2><p>Quản lý hồ sơ và mở kế hoạch giáo dục riêng cho từng trẻ.</p></div></div><div className="children-grid">{childList.map((child, childIndex) => { const dayLabel = TEACHING_DAYS.filter((day) => child.teachingDays?.includes(day.value)).map((day) => day.label).join(", ") || "Chưa cài"; const colorOption = SCHEDULE_COLOR_OPTIONS.find((option) => option.value === child.color) ?? SCHEDULE_COLOR_OPTIONS[childIndex % SCHEDULE_COLOR_OPTIONS.length]; const timeLabel = child.teachingStartTime && child.teachingEndTime ? `${child.teachingStartTime} - ${child.teachingEndTime}` : "Chưa cài"; return <article className="child-card" key={child.id} role="button" tabIndex={0} onClick={() => onSelectPlan(child.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelectPlan(child.id); } }}><div className="child-card-head"><ChildAvatar name={child.name} /><div><h3>{child.name}</h3><p>{child.gender} · Sinh ngày {child.birthday}</p></div></div><div className="child-card-details"><span><Icon name="calendar" size={15} /> Ngày dạy: {dayLabel}</span><span><Icon name="calendar" size={15} /> Giờ dạy: {timeLabel}</span><span><i className={`schedule-color-swatch ${colorOption.value}`} /> Màu lịch: {colorOption.label}</span></div><div className="child-card-note"><Icon name="note" size={17} />{child.note || "Chưa có ghi chú"}</div><div className="child-card-actions"><div className="child-card-links"><button type="button" className="link-button" onClick={(event) => { event.stopPropagation(); onSelectPlan(child.id); }}>Xem kế hoạch <span>→</span></button><button type="button" className="link-button share-link-button" onClick={(event) => { event.stopPropagation(); copyChildShareLink(child); }}><Icon name="share" size={15} />Chia sẻ</button></div><div><button type="button" className="icon-action edit" onClick={(event) => { event.stopPropagation(); onEdit(child); }} aria-label={`Chỉnh sửa ${child.name}`}><Icon name="edit" size={18} /></button><button type="button" className="icon-action delete" onClick={(event) => { event.stopPropagation(); onDelete(child.id); }} aria-label={`Xóa ${child.name}`}><Icon name="trash" size={18} /></button></div></div></article>; })}</div>{childList.length === 0 && <div className="empty-state"><Icon name="children" size={34} /><h3>Chưa có hồ sơ trẻ</h3><p>Nhấn “Thêm trẻ” để nhập hồ sơ đầu tiên.</p></div>}</>;
}
...
type GoalDialogMode = "domain" | "long" | "short" | "edit-domain" | "edit-domain-setting" | "edit-long" | "period" | "edit-short" | "edit-period";
...
function ReadOnlyStatus({ value }: { value: Status }) {
  ...
  return <div className={`status-readonly ${value === "Đạt" ? "achieved" : value === "Manh nha" ? "emerging" : "not-achieved"}`}><span className="status-dot" /><span>{value}</span></div>;
}
...
function GoalDialog({ mode, goals, domains, evaluationPeriods, editingGoal, initialValue = "", initialGoalId, initialDomain, initialPeriodLabel = "", initialIcon, onCancel, onSave }: { mode: GoalDialogMode; goals: Goal[]; domains: string[]; evaluationPeriods: string[]; editingGoal?: Goal; initialValue?: string; initialGoalId?: number; initialDomain?: string; initialPeriodLabel?: string; initialIcon?: DomainIcon; onCancel: () => void; onSave: (data: GoalDialogSave) => void }) {
  ...
  const [status, setStatus] = useState<Status>((initialValue as Status) ?? "");
  ...
  const [periodLabels, setPeriodLabels] = useState<string[]>(evaluationPeriods);
  ...
  const [resultStatuses, setResultStatuses] = useState<Status[]>(editingGoal?.statuses ?? evaluationPeriods.map(() => ""));
  ...
  const title = mode === "domain" ? "Thêm lĩnh vực" : mode === "edit-domain" ? "Chỉnh sửa lĩnh vực" : isSettingsDomainEdit ? "Chỉnh sửa lĩnh vực" : mode === "period" ? "Thêm thời gian kết quả" : mode === "long" ? "Thêm mục tiêu dài hạn" : mode === "short" ? "Thêm mục tiêu ngắn hạn" : mode === "edit-long" ? "Chỉnh sửa mục tiêu dài hạn" : mode === "edit-short" ? "Chỉnh sửa mục tiêu ngắn hạn" : "Chỉnh sửa kết quả theo tuần";
  const description = mode === "domain" ? "Chọn lĩnh vực đã được cấu hình trong Cài đặt." : isSettingsDomainEdit ? "Cập nhật tên và icon hiển thị của lĩnh vực." : isDomainEdit ? "Cập nhật nhanh toàn bộ thông tin của lĩnh vực này." : isPeriodEdit ? "Cập nhật tên thời gian và trạng thái của tuần đang chọn." : mode === "period" ? "Thêm một mốc thời gian để theo dõi kết quả." : "Thông tin sẽ được hiển thị đồng thời ở Kế hoạch giáo dục và Tổng quan.";
  ...
  const canSave = isSettingsDomainEdit || isDomainEdit ? Boolean(domainName.trim()) : isPeriodEdit ? Boolean(periodLabel.trim()) : mode === "domain" ? Boolean(domain && longTerm.trim()) : Boolean(text.trim());
  ...
  const handleSave = () => {
    ...
    if (isDomainEdit && (nextPeriodLabels.some((item) => !item) || new Set(nextPeriodLabels.map((item) => item.toLocaleLowerCase())).size !== nextPeriodLabels.length)) {
      window.alert("Tên thời gian theo tuần phải đầy đủ và không được trùng nhau.");
      return;
    }
    onSave({ mode, text: isDomainEdit ? longTerm.trim() : isSettingsDomainEdit ? domainName.trim() : isPeriodEdit ? status : mode === "domain" ? longTerm.trim() : text.trim(), domain: isDomainEdit || isSettingsDomainEdit ? domainName.trim() : domain, domainIcon: isSettingsDomainEdit ? domainIcon : undefined, shortTerm: isDomainEdit ? shortTerms.map((item) => item.trim()).filter(Boolean) : undefined, statuses: isDomainEdit ? resultStatuses : undefined, note: isDomainEdit ? note.trim() : undefined, periodLabel: isPeriodEdit ? periodLabel.trim() : undefined, periodLabels: isDomainEdit ? nextPeriodLabels : undefined, goalId: mode === "short" ? Number(goalId) : editingGoal?.id });
  };
  return <div className={`modal-backdrop ${isDomainEdit || isDomainAdd || isSettingsDomainEdit ? "domain-edit-modal" : isPeriodEdit || isEdit || mode === "long" || mode === "period" ? "small-edit-modal" : ""}`} role="presentation"><div className="modal-card goal-dialog" role="dialog" aria-modal="true" aria-labelledby="goal-dialog-title"><div className="modal-head"><div><h2 id="goal-dialog-title">{title}</h2><p>{description}</p></div><button type="button" className="close-button" onClick={onCancel} aria-label="Đóng">×</button></div><div className="form-grid">{(mode === "domain" || mode === "long") && <SelectField label="Lĩnh vực" value={domain} onChange={setDomain} options={domainOptions} required />}{mode === "domain" && <label className="field full"><span>Mục tiêu dài hạn<em>*</em></span><textarea value={longTerm} onChange={(event) => setLongTerm(event.target.value)} placeholder="Nhập mục tiêu dài hạn..." /></label>}{mode === "short" && <label className="field"><span>Mục tiêu dài hạn<em>*</em></span><div className="select-wrap"><select value={goalId} onChange={(event) => setGoalId(event.target.value)}>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.longTerm || "Chưa nhập mục tiêu"}</option>)}</select><Icon name="chevron" size={18} /></div></label>}{isSettingsDomainEdit && <><InputField label="Tên lĩnh vực" value={domainName} onChange={setDomainName} required /><label className="field full"><span>Icon lĩnh vực<em>*</em></span><DomainIconPicker value={domainIcon} onChange={setDomainIcon} /></label></>}{isDomainEdit && <><SelectField label="Tên lĩnh vực" value={domainName} onChange={setDomainName} options={domainOptions} required /><label className="field full"><span>Mục tiêu dài hạn</span><input className="goal-long-term-input" list="goal-dialog-long-term-options" value={longTerm} onChange={(event) => setLongTerm(event.target.value)} placeholder="Chọn hoặc nhập mục tiêu dài hạn..." /><datalist id="goal-dialog-long-term-options">{[...new Set(goals.map((goal) => goal.longTerm.trim()).filter(Boolean))].map((option) => <option key={option} value={option} />)}</datalist></label><label className="field full"><span>Mục tiêu ngắn hạn</span><div className="quick-short-list">{shortTerms.map((item, index) => <div className="quick-short-row" key={index}><input value={item} onChange={(event) => updateShortTerm(index, event.target.value)} placeholder={`Mục tiêu ngắn hạn ${index + 1}`} /><button type="button" className="row-action delete" onClick={() => setShortTerms((items) => items.length > 1 ? items.filter((_, itemIndex) => itemIndex !== index) : items)} aria-label="Xóa mục tiêu ngắn hạn"><Icon name="trash" size={16} /></button></div>)}</div><button type="button" className="outline-button compact quick-add-button" onClick={() => setShortTerms((items) => [...items, ""])}><Icon name="plus" size={15} />Thêm mục tiêu ngắn hạn</button></label><label className="field full"><span>Kết quả theo tuần</span><div className="quick-results-list">{periodLabels.map((period, index) => <div className="quick-result-row" key={`${period}-${index}`}><input className="quick-period-label" value={period} onChange={(event) => updatePeriodLabel(index, event.target.value)} aria-label={`Tên thời gian ${period}`} /><div className="select-wrap"><select value={resultStatuses[index] ?? ""} onChange={(event) => setResultStatuses((items) => items.map((item, itemIndex) => itemIndex === index ? event.target.value as Status : item))}>{STATUS_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select><Icon name="chevron" size={16} /></div><button type="button" className="row-action delete" onClick={() => removePeriodLabel(index)} disabled={periodLabels.length <= 1} aria-label={`Xóa ${period}`}><Icon name="trash" size={16} /></button></div>)}</div><button type="button" className="outline-button compact quick-add-button" onClick={addPeriodLabel}><Icon name="plus" size={15} />Thêm kết quả theo tuần</button></label><label className="field full"><span>Ghi chú</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Nhập ghi chú..." /></label></>}{isPeriodEdit && <><label className="field full"><span>Tên thời gian<em>*</em></span><input value={periodLabel} onChange={(event) => setPeriodLabel(event.target.value)} placeholder={placeholder} /></label><label className="field full"><span>Trạng thái kết quả<em>*</em></span><div className="select-wrap"><select value={status} onChange={(event) => setStatus(event.target.value as Status)}>{STATUS_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select><Icon name="chevron" size={18} /></div></label></>}{mode !== "short" && mode !== "domain" && !isPeriodEdit && !isDomainEdit && !isSettingsDomainEdit && <label className="field full"><span>{label}<em>*</em></span>{mode === "period" ? <input value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} /> : <textarea value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} />}</label>}{mode === "short" && <label className="field full"><span>{label}<em>*</em></span><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder={placeholder} /></label>}</div><div className="modal-actions"><button type="button" className="button" onClick={onCancel}>Hủy</button><button type="button" className="button primary" disabled={!canSave} onClick={handleSave}><Icon name="save" size={17} />Lưu</button></div></div></div>;
}

function NoteDialog({ goal, onCancel, onSave }: { goal?: Goal; onCancel: () => void; onSave: (note: string) => void }) {
  const [note, setNote] = useState(goal?.note ?? "");
  return <div className="modal-backdrop" role="presentation"><div className="modal-card note-dialog" role="dialog" aria-modal="true" aria-labelledby="note-dialog-title"><div className="modal-head"><div><h2 id="note-dialog-title">Ghi chú mục tiêu</h2><p>{goal?.longTerm || "Nhập ghi chú cho mục tiêu đang theo dõi."}</p></div><button type="button" className="close-button" onClick={onCancel} aria-label="Đóng">×</button></div><label className="field"><span>Ghi chú</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Nhập ghi chú..." /></label><div className="modal-actions"><button type="button" className="button" onClick={onCancel}>Hủy</button><button type="button" className="button primary" onClick={() => onSave(note.trim())}><Icon name="save" size={17} />Lưu ghi chú</button></div></div></div>;
}

function PlanNewDomainCard({ domain, childId, goals, periods, domainIcon, isOpen, search, onToggle, onAddLong, onAddShort, onAddPeriod, onEditDomain, onEditLong, onDeleteGoal, onEditShort, onDeleteShort, onEditPeriod, onDeleteDomain, onStatusChange, onNoteClick }: { domain: string; childId: number; goals: Goal[]; periods: string[]; domainIcon: DomainIcon; isOpen: boolean; search: string; onToggle: () => void; onAddLong: (domain: string) => void; onAddShort: (goal: Goal) => void; onAddPeriod: () => void; onEditDomain: (goal: Goal) => void; onEditLong: (goal: Goal) => void; onDeleteGoal: (goal: Goal) => void; onEditShort: (goal: Goal, index: number) => void; onDeleteShort: (goal: Goal, index: number) => void; onEditPeriod: (goal: Goal, index: number) => void; onDeleteDomain: (domain: string) => void; onStatusChange: (id: number, periodIndex: number, status: Status) => void; onNoteClick: (id: number) => void }) {
  const normalizedSearch = search.trim().toLowerCase();
  const visibleGoals = goals.filter((goal) => !normalizedSearch || `${goal.longTerm} ${goal.shortTerm.join(" ")}`.toLowerCase().includes(normalizedSearch));
  const editTarget = goals[0] ?? { id: 0, childId, domain, longTerm: "", shortTerm: [], from: "01/07/2026", to: "30/08/2026", statuses: periods.map(() => "" as Status) };
  const periodLabels = periods.length ? periods : DEFAULT_EVALUATION_PERIODS;
  return <article className={`plan-new-domain ${isOpen ? "is-open" : "is-collapsed"}`}>
    <header className="plan-new-domain-head" role="button" tabIndex={0} onClick={(event) => { if (!(event.target as HTMLElement).closest("button")) onToggle(); }} onKeyDown={(event) => { if ((event.key === "Enter" || event.key === " ") && !(event.target as HTMLElement).closest("button")) { event.preventDefault(); onToggle(); } }}>
      <div className="plan-new-domain-title">
        <DomainIconBadge value={domainIcon} size={21} />
        <div><span className="plan-new-eyebrow">Lĩnh vực</span><h3>{domain}</h3><p>{goals.length} mục tiêu dài hạn <span>·</span> {goals.length * periodLabels.length} kết quả theo tuần</p></div>
      </div>
      <div className="plan-new-domain-actions">
        <button className="plan-new-icon-action" type="button" onClick={() => onEditDomain(editTarget)} aria-label={`Sửa lĩnh vực ${domain}`}><Icon name="edit" size={18} /></button>
        <button className="plan-new-icon-action danger" type="button" onClick={() => onDeleteDomain(domain)} aria-label={`Xóa lĩnh vực ${domain}`}><Icon name="trash" size={18} /></button>
        <button className="plan-new-toggle" type="button" onClick={onToggle} aria-expanded={isOpen} aria-label={`${isOpen ? "Thu gọn" : "Mở rộng"} lĩnh vực ${domain}`}><Icon name="chevron" size={19} /></button>
      </div>
    </header>
    {isOpen && <div className="plan-new-domain-body">
      {visibleGoals.length ? visibleGoals.map((goal, goalIndex) => <article className="plan-new-goal" key={goal.id}>
        <div className="plan-new-goal-head">
        <div><span className="plan-new-goal-label">Mục tiêu dài hạn {String(goalIndex + 1).padStart(2, "0")}</span><h4>{goal.longTerm || <span className="cell-placeholder">Chưa nhập mục tiêu dài hạn</span>}</h4></div>
          <div className="plan-new-goal-actions"><button className="plan-new-small-action" type="button" onClick={() => onEditLong(goal)}><Icon name="edit" size={15} />Sửa</button><button className="plan-new-small-action" type="button" onClick={() => onNoteClick(goal.id)} aria-label={`Ghi chú mục tiêu ${goal.longTerm || goal.domain}`}><Icon name="note" size={15} />Ghi chú</button><button className="plan-new-small-action danger" type="button" onClick={() => onDeleteGoal(goal)} aria-label="Xóa mục tiêu dài hạn"><Icon name="trash" size={15} /></button></div>
        </div>
        <div className="plan-new-goal-content">
          <section className="plan-new-short-section"><div className="plan-new-section-head"><h5>Mục tiêu ngắn hạn</h5><button className="plan-new-outline-action" type="button" onClick={() => onAddShort(goal)}><Icon name="plus" size={15} />Thêm</button></div>{goal.shortTerm.length ? <ul>{goal.shortTerm.map((item, index) => <li key={`${goal.id}-short-${index}`}><span className="plan-new-bullet" /><span>{item || <span className="cell-placeholder">Chưa nhập mục tiêu</span>}</span><span className="plan-new-row-actions"><button type="button" onClick={() => onEditShort(goal, index)} aria-label="Sửa mục tiêu ngắn hạn"><Icon name="edit" size={16} /></button><button type="button" className="danger" onClick={() => onDeleteShort(goal, index)} aria-label="Xóa mục tiêu ngắn hạn"><Icon name="trash" size={16} /></button></span></li>)}</ul> : <p className="plan-new-muted">Chưa có mục tiêu ngắn hạn.</p>}</section>
          <section className="plan-new-result-section"><div className="plan-new-section-head"><div><h5>Kết quả theo tuần</h5><p>Cập nhật trạng thái theo từng giai đoạn</p></div><button className="plan-new-outline-action" type="button" onClick={onAddPeriod}><Icon name="plus" size={15} />Thêm thời gian</button></div><div className="plan-new-period-grid">{periodLabels.map((label, periodIndex) => <div className="plan-new-period" key={`${goal.id}-${label}-${periodIndex}`}><div className="plan-new-period-label"><span>{label}</span><button type="button" onClick={() => onEditPeriod(goal, periodIndex)} aria-label={`Sửa kết quả ${label}`}><Icon name="edit" size={15} /></button></div><EditableStatus value={goal.statuses[periodIndex] ?? ""} onChange={(status) => onStatusChange(goal.id, periodIndex, status)} /></div>)}</div></section>
        </div>
        <footer className="plan-new-goal-footer"><span>Ghi chú: {goal.note || "Chưa có ghi chú"}</span></footer>
      </article>) : <div className="plan-new-empty"><Icon name="target" size={24} /><div><strong>{normalizedSearch ? "Không tìm thấy mục tiêu phù hợp" : "Chưa có mục tiêu dài hạn"}</strong><span>{normalizedSearch ? "Thử từ khóa khác hoặc xóa bộ lọc." : "Bắt đầu bằng cách thêm mục tiêu dài hạn cho lĩnh vực này."}</span></div></div>}
      <div className="plan-new-domain-footer"><span>{goals.length} mục tiêu dài hạn</span><button className="plan-new-primary-small" type="button" onClick={() => onAddLong(domain)}><Icon name="plus" size={15} />Thêm mục tiêu dài hạn</button></div>
    </div>}
  </article>;
}

function PlanNewView({ childList, selectedChildId, onSelectChild, goals, hiddenDomains, evaluationPeriods, domainIcons, initialOpenDomains, onOpenDomainsChange, onSave, onAddDomain, onAddLong, onAddShort, onAddPeriod, onEditDomain, onEditLong, onDeleteGoal, onEditShort, onDeleteShort, onEditPeriod, onDeleteDomain, onStatusChange, onNoteClick }: { childList: Child[]; selectedChildId: number; onSelectChild: (id: number) => void; goals: Goal[]; hiddenDomains: string[]; evaluationPeriods: string[]; domainIcons: Record<string, DomainIcon>; initialOpenDomains: Record<string, boolean>; onOpenDomainsChange: (openDomains: Record<string, boolean>) => void; onSave: (openDomains: Record<string, boolean>) => void | Promise<void>; onAddDomain: () => void; onAddLong: (domain?: string) => void; onAddShort: (goal: Goal) => void; onAddPeriod: () => void; onEditDomain: (goal: Goal) => void; onEditLong: (goal: Goal) => void; onDeleteGoal: (goal: Goal) => void; onEditShort: (goal: Goal, index: number) => void; onDeleteShort: (goal: Goal, index: number) => void; onEditPeriod: (goal: Goal, index: number) => void; onDeleteDomain: (domain: string) => void; onStatusChange: (id: number, periodIndex: number, status: Status) => void; onNoteClick: (id: number) => void }) {
  const child = childList.find((item) => item.id === selectedChildId) ?? childList[0];
  const childGoals = child ? goals.filter((goal) => goal.childId === child.id) : [];
  const [search, setSearch] = useState("");
  const [domainFilter, setDomainFilter] = useState("");
  const [openDomains, setOpenDomains] = useState<Record<string, boolean>>(() => initialOpenDomains);
  const updateOpenDomains = (next: Record<string, boolean>) => { setOpenDomains(next); onOpenDomainsChange(next); };
  const visibleChildGoals = useMemo(() => childGoals.filter((goal) => !hiddenDomains.includes(goal.domain)), [childGoals, hiddenDomains]);
  const domainNames = useMemo(() => [...new Set(visibleChildGoals.map((goal) => goal.domain))].filter((domain) => !hiddenDomains.includes(domain)), [visibleChildGoals, hiddenDomains]);
  const visibleDomainNames = domainNames.filter((domain) => {
    const matchesFilter = !domainFilter || domain === domainFilter;
    const domainGoals = visibleChildGoals.filter((goal) => goal.domain === domain);
  ...
  return <div className="plan-new-view">
    ...
    <div className="plan-new-section-title"><div><h2><Icon name="calendar" size={21} />MỤC TIÊU PHÁT TRIỂN</h2><p>Thiết lập và theo dõi mục tiêu riêng cho {child.name}.</p></div><div className="mini-legend"><span><i className="green-dot" />Đạt (Đ)</span><span><i className="yellow-dot" />Manh nha (MN)</span><span><i className="gray-dot" />Chưa đạt (CĐ)</span></div></div>
    <section className="plan-new-board">
      <div className="plan-new-toolbar"><div className="plan-new-search"><Icon name="overview" size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm kiếm lĩnh vực, mục tiêu..." aria-label="Tìm kiếm lĩnh vực, mục tiêu" /></div><select value={domainFilter} onChange={(event) => setDomainFilter(event.target.value)} aria-label="Lọc theo lĩnh vực"><option value="">Tất cả lĩnh vực</option>{domainNames.map((domain) => <option key={domain} value={domain}>{domain}</option>)}</select><span className="plan-new-toolbar-count"><strong>{visibleDomainNames.length}</strong> lĩnh vực</span><button className="button primary" type="button" onClick={onAddDomain}><Icon name="plus" size={17} />Thêm lĩnh vực</button></div>
      <div className="plan-new-board-actions"><button className="outline-button compact" type="button" onClick={() => updateOpenDomains(Object.fromEntries(domainNames.map((domain) => [domain, false])))}><Icon name="chevron" size={15} />Thu gọn tất cả</button><button className="outline-button compact" type="button" onClick={() => updateOpenDomains(Object.fromEntries(domainNames.map((domain) => [domain, true])))}><Icon name="chevron" size={15} />Mở tất cả</button></div>
      <div className="plan-new-domain-list">{visibleDomainNames.map((domain, index) => <PlanNewDomainCard key={domain} domain={domain} childId={child.id} goals={visibleChildGoals.filter((goal) => goal.domain === domain)} periods={evaluationPeriods} domainIcon={domainIcons[domain] ?? DEFAULT_DOMAIN_ICONS[domain] ?? DOMAIN_ICON_OPTIONS[0].value} isOpen={openDomains[domain] ?? index === 0} search={search} onToggle={() => updateOpenDomains({ ...openDomains, [domain]: !(openDomains[domain] ?? index === 0) })} onAddLong={onAddLong} onAddShort={onAddShort} onAddPeriod={onAddPeriod} onEditDomain={onEditDomain} onEditLong={onEditLong} onDeleteGoal={onDeleteGoal} onEditShort={onEditShort} onDeleteShort={onDeleteShort} onEditPeriod={onEditPeriod} onDeleteDomain={onDeleteDomain} onStatusChange={onStatusChange} onNoteClick={onNoteClick} />)}</div>
      ...
      <footer className="plan-new-footer"><span>Hiển thị {visibleDomainNames.length} lĩnh vực · {visibleChildGoals.length} mục tiêu dài hạn</span><span>Gợi ý: mở từng lĩnh vực để thao tác nhanh, tránh màn hình quá dày.</span></footer>
    </section>
  </div>;
}
...
function OverviewCalendar({ childList }: { childList: Child[] }) {
  ...
  const [popup, setPopup] = useState<{ date: Date; entries: Array<{ child: Child; time: string; color: string }> } | null>(null);
  ...
  const schedules = useMemo(() => childList.flatMap((child, childIndex) => (child.teachingDays ?? []).map((day) => ({ child, day, time: child.teachingStartTime && child.teachingEndTime ? `${child.teachingStartTime} - ${child.teachingEndTime}` : "Chưa có giờ dạy", color: child.color ?? SCHEDULE_COLOR_OPTIONS[childIndex % SCHEDULE_COLOR_OPTIONS.length].value }))), [childList]);
  ...
  return <>
    <section className="overview-calendar" aria-label="Lịch kế hoạch">
      ...
      <div className="overview-calendar-days">{cells.map((cell, index) => {
        ...
        const classes = [cell.muted ? "is-muted" : "", isSunday ? "is-sunday" : "", isScheduled ? "is-scheduled" : "", entries.length > 1 ? "is-multi-scheduled" : "", entries.length === 1 ? `schedule-color-${entries[0].color}` : "", isToday ? "is-today" : ""].filter(Boolean).join(" ");
        const openPopup = () => setPopup({ date: cell.date, entries });
        return <span key={`${key}-${index}`} className={classes} role={isScheduled ? "button" : undefined} tabIndex={isScheduled ? 0 : undefined} onClick={isScheduled ? openPopup : undefined} onKeyDown={isScheduled ? (event) => { if (event.key === "Enter" || event.key === " ") openPopup(); } : undefined}><b>{cell.day}</b>{entries.length === 1 && <><small className={`calendar-event-name schedule-text-${entries[0].color}`}>{calendarChildName(entries[0].child.name)}</small><small className="calendar-event-time">{entries[0].time}</small></>}{entries.length > 1 && entries.map((entry) => <small className={`calendar-event-name schedule-text-${entry.color}`} key={entry.child.id}>{calendarChildName(entry.child.name)}</small>)}</span>;
      })}</div>
    </section>
    {popup && <div className="calendar-popup-backdrop" role="presentation"><div className="calendar-schedule-popup" role="dialog" aria-modal="true" aria-labelledby="calendar-popup-title"><div className="calendar-popup-head"><div><span>LỊCH DẠY</span><h2 id="calendar-popup-title">{formatPopupDate(popup.date)}</h2></div><button type="button" className="close-button" onClick={() => setPopup(null)} aria-label="Đóng">×</button></div><div className="calendar-popup-list">{popup.entries.map((entry) => <div className="calendar-popup-entry" key={entry.child.id}><i className={`schedule-dot schedule-color-${entry.color}`} /><div><strong>{entry.child.name}</strong><span>{entry.time}</span></div></div>)}</div></div></div>}
  </>;
}

function OverviewViewV2({ childList, selectedChildId, onSelectChild, goals, evaluationPeriods, domainIcons, onOpenPlan }: { childList: Child[]; selectedChildId: number; onSelectChild: (id: number) => void; goals: Goal[]; evaluationPeriods: string[]; domainIcons: Record<string, DomainIcon>; onOpenPlan: () => void }) {
  const child = childList.find((item) => item.id === selectedChildId) ?? childList[0];
  ...
  return <><Header title="Tổng quan" subtitle="Theo dõi nhanh kế hoạch giáo dục của các trẻ" actionLabel="Xem kế hoạch" actionIcon="file" onAction={onOpenPlan} /><OverviewCalendar childList={childList} /><div className="overview-toolbar"><ChildChoiceButtons label="Đang xem tổng quan của" childList={childList} selectedChildId={selectedChildId} onSelectChild={onSelectChild} /></div><ChildSummary child={child} /><section className="overview-goals-panel"><div className="section-title-row"><div><h2><Icon name="overview" size={21} /> Mục tiêu đang theo dõi</h2><p>Thông tin chỉ hiển thị; chỉnh sửa tại Kế hoạch giáo dục.</p></div><div className="mini-legend"><span><i className="green-dot" />Đạt</span><span><i className="yellow-dot" />Manh nha</span><span><i className="gray-dot" />Chưa đạt</span></div></div><GoalsTableV2 goals={childGoals} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} readOnly /></section></>;
}

function ShareView({ child, goals, evaluationPeriods, domainIcons }: { child: Child; goals: Goal[]; evaluationPeriods: string[]; domainIcons: Record<string, DomainIcon> }) {
  const achievedCount = goals.reduce((total, goal) => total + goal.statuses.filter((status) => status === "Đạt").length, 0);
  return <main className="share-page"><div className="share-container"><header className="share-header"><div className="share-brand"><span className="share-brand-mark"><Icon name="target" size={20} /></span><div><strong>KẾ HOẠCH GIÁO DỤC</strong><small>Trang chia sẻ hồ sơ trẻ</small></div></div><span className="share-readonly"><Icon name="file" size={16} />Chỉ xem</span></header><section className="share-hero"><span className="share-eyebrow">HỒ SƠ TRẺ</span><h1>{child.name}</h1><p>Thông tin kế hoạch giáo dục được chia sẻ riêng cho hồ sơ này.</p></section><ChildSummary child={child} /><div className="share-summary"><span><strong>{goals.length}</strong> mục tiêu đang theo dõi</span><span><strong>{achievedCount}</strong> kết quả đạt</span></div><section className="share-goals overview-goals-panel"><div className="share-section-heading"><div><span className="share-eyebrow">KẾ HOẠCH GIÁO DỤC</span><h2>Mục tiêu phát triển</h2><p>Kết quả được hiển thị theo từng giai đoạn đánh giá.</p></div><span className="share-lock"><Icon name="file" size={16} />Chế độ chỉ xem</span></div><GoalsTableV2 goals={goals} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} readOnly /></section><footer className="share-footer">Đường dẫn này chỉ hiển thị thông tin của <strong>{child.name}</strong>.</footer></div></main>;
}

/* eslint-enable jsx-a11y/no-autofocus */

let settingsChildList: Child[] = [];
let settingsSelectedChildId = 0;
let settingsExportPdfHandler: (child: Child) => void = () => undefined;
let settingsExportWordHandler: (child: Child) => void = () => undefined;

function SettingsView({ darkMode, onToggleTheme, domains = DOMAIN_OPTIONS, domainIcons = DEFAULT_DOMAIN_ICONS, childList = settingsChildList, selectedChildId = settingsSelectedChildId, onExportPdf = (child) => settingsExportPdfHandler(child), onExportWord = (child) => settingsExportWordHandler(child), onAddDomain = () => undefined, onEditDomain = () => undefined, onDeleteDomain = () => undefined }: { darkMode: boolean; onToggleTheme: () => void; domains?: string[]; domainIcons?: Record<string, DomainIcon>; childList?: Child[]; selectedChildId?: number; onExportPdf?: (child: Child) => void; onExportWord?: (child: Child) => void; onAddDomain?: (domain: string, icon: DomainIcon) => void; onEditDomain?: (domain: string) => void; onDeleteDomain?: (domain: string) => void }) {
  ...
  const [exportChildId, setExportChildId] = useState(String(selectedChildId || childList[0]?.id || ""));
  const selectedChild = childList.find((child) => child.id === Number(exportChildId)) ?? childList.find((child) => child.id === selectedChildId) ?? childList[0];
  ...
  const openExportDialog = (type: "pdf" | "word") => { setExportChildId(String(selectedChildId || childList[0]?.id || "")); setExportType(type); };
  const confirmExport = () => { const child = childList.find((item) => item.id === Number(exportChildId)); if (!child || !exportType) return; if (exportType === "pdf") onExportPdf(child); else onExportWord(child); setExportType(null); };
  return <>{exportType && <div className="modal-backdrop" role="presentation"><div className="modal-card export-child-modal" role="dialog" aria-modal="true" aria-labelledby="export-child-title"><div className="modal-head"><div><span className="modal-kicker">XUẤT HỒ SƠ CHIA SẺ</span><h2 id="export-child-title">Chọn trẻ để xuất {exportType === "pdf" ? "PDF" : "Word"}</h2></div><button type="button" className="icon-button" onClick={() => setExportType(null)} aria-label="Đóng">×</button></div><label className="field"><span>Hồ sơ trẻ</span><div className="select-wrap"><select value={exportChildId} onChange={(event) => setExportChildId(event.target.value)}>{childList.map((child) => <option key={child.id} value={child.id}>{child.name}</option>)}</select><Icon name="chevron" size={17} /></div></label><p className="modal-description">File sẽ giữ nguyên nội dung, icon lĩnh vực và bố cục màn hình share; trang được đặt theo chiều ngang.</p><div className="modal-actions"><button type="button" className="button" onClick={() => setExportType(null)}>Hủy</button><button type="button" className="button primary" onClick={confirmExport} disabled={!childList.length}>{exportType === "pdf" ? "Xuất PDF" : "Xuất Word"}</button></div></div></div>}<Header title="Cài đặt" subtitle="Tùy chỉnh cách bạn sử dụng kế hoạch giáo dục" /><div className="settings-card"><div className="settings-heading"><div className="settings-icon"><Icon name="settings" size={24} /></div><div><h2>Tùy chọn ứng dụng</h2><p>Các thay đổi được lưu trên thiết bị này.</p></div></div><div className="setting-row"><span><strong>Giao diện tối</strong><small>Đổi sang nền tối để sử dụng dễ chịu hơn vào buổi tối.</small></span><input aria-label="Giao diện tối" type="checkbox" checked={darkMode} onChange={onToggleTheme} /></div></div><div className="settings-card export-settings-card"><div className="settings-heading"><div className="settings-icon export-settings-icon"><Icon name="file" size={24} /></div><div><h2>Xuất hồ sơ chia sẻ</h2><p>Chọn đúng hồ sơ trẻ trước khi xuất file share.</p></div></div><div className="export-settings-actions"><button type="button" className="button primary" disabled={!childList.length} onClick={() => openExportDialog("pdf")}><Icon name="file" size={17} />Xuất PDF</button><button type="button" className="button" disabled={!childList.length} onClick={() => openExportDialog("word")}><Icon name="file" size={17} />Xuất Word</button></div>{!childList.length && <small className="settings-help-text">Hãy thêm hồ sơ trẻ trước khi xuất file.</small>}</div><div className="settings-card domain-settings-section"><div className="settings-heading"><div className="settings-icon"><Icon name="target" size={24} /></div><div><h2>Quản lý lĩnh vực</h2><p>Đặt tên và chọn icon riêng cho từng lĩnh vực phát triển.</p></div></div><form className="domain-settings-form" onSubmit={submitDomain}><InputField label="Tên lĩnh vực mới" value={newDomain} onChange={setNewDomain} placeholder="Ví dụ: Kỹ năng tự phục vụ" required /><label className="field domain-settings-icon-field"><span>Icon lĩnh vực</span><DomainIconPicker value={newDomainIcon} onChange={setNewDomainIcon} /></label><button type="submit" className="button primary"><Icon name="plus" size={17} />Thêm lĩnh vực</button></form><div className="domain-settings-list" aria-live="polite">{domains.map((domain) => <div className="domain-settings-item" key={domain}><DomainIconBadge value={domainIcons[domain]} size={19} /><span>{domain}</span><button type="button" className="domain-settings-action edit" onClick={() => onEditDomain(domain)} aria-label={`Sửa ${domain}`}><Icon name="edit" size={17} /></button><button type="button" className="domain-settings-action delete" onClick={() => onDeleteDomain(domain)} aria-label={`Xóa ${domain}`}><Icon name="trash" size={17} /></button></div>)}</div></div></>;
}
...
export default function Home() {
  ...
  const [planNewHiddenDomainsByChild, setPlanNewHiddenDomainsByChild] = useState<Record<string, string[]>>(() => {
    if (typeof window === "undefined") return {};
    try {
      const parsed = JSON.parse(window.localStorage.getItem(PLAN_NEW_HIDDEN_DOMAINS_STORAGE_KEY) || "{}");
      if (!parsed || typeof parsed !== "object") return {};
      return Object.fromEntries(Object.entries(parsed).map(([childId, values]) => [childId, Array.isArray(values) ? values.filter((value): value is string => typeof value === "string") : []]));
    } catch {
      return {};
    }
  });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (active instanceof HTMLElement && active.closest(".modal-backdrop")) active.blur();
    }, 0);
    return () => {
  ...
  }, [showChildForm, goalDialog, noteGoalId]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    loadCloudData<AppCloudData>().then((data) => {
      if (!active) return;
      const { children: cloudChildren, goals: cloudGoals, evaluationPeriods: cloudPeriods, evaluationPeriodsByChild: cloudPeriodsByChild, domains: cloudDomains, domainIcons: cloudDomainIcons, collapsedGoalIds: cloudCollapsedGoalIds, planNewOpenDomainsByChild: cloudPlanNewOpenDomainsByChild, planNewOpenDomains: cloudPlanNewOpenDomains, ...extras } = data ?? {};
      ...
      if (cloudPlanNewOpenDomainsByChild && typeof cloudPlanNewOpenDomainsByChild === "object") setPlanNewOpenDomainsByChild(cloudPlanNewOpenDomainsByChild as Record<string, Record<string, boolean>>);
      else if (cloudPlanNewOpenDomains && typeof cloudPlanNewOpenDomains === "object" && nextChildren[0]) setPlanNewOpenDomainsByChild({ [String(nextChildren[0].id)]: cloudPlanNewOpenDomains as Record<string, boolean> });
      setCloudReady(true);
    }).catch((error) => {
  ...
  useEffect(() => {
    ...
    cloudSaveTimer.current = window.setTimeout(() => {
      saveCloudData({ ...cloudExtras, children, goals, evaluationPeriods: currentEvaluationPeriods, evaluationPeriodsByChild, domains, domainIcons, collapsedGoalIds, planNewOpenDomainsByChild }).catch((error) => console.error("Không thể đồng bộ dữ liệu lên Google Sheet:", error));
    }, 250);
    return () => { if (cloudSaveTimer.current !== null) window.clearTimeout(cloudSaveTimer.current); };
  }, [children, goals, evaluationPeriods, currentEvaluationPeriods, evaluationPeriodsByChild, domains, domainIcons, collapsedGoalIds, planNewOpenDomainsByChild, cloudExtras, cloudReady, shareChildSlug]);
  ...
  const updateStatus = (id: number, periodIndex: number, status: Status) => setGoals((items) => items.map((goal) => {
    ...
    return { ...goal, statuses: periods.map((_, index) => index === periodIndex ? status : goal.statuses[index] ?? "") };
  }));
  ...
  const saveGoal = (goal: Omit<Goal, "id">) => { const childPeriods = periodsForChild(goal.childId); const normalizedGoal = { ...goal, statuses: childPeriods.map((_, index) => goal.statuses[index] ?? "") }; setEvaluationPeriodsByChild((items) => items[String(goal.childId)] ? items : { ...items, [String(goal.childId)]: [...childPeriods] }); setGoals((items) => [...items, { ...normalizedGoal, id: Math.max(0, ...items.map((item) => item.id)) + 1 }]); setSelectedChildId(goal.childId); setView("plan"); };
  const updatePlanNewOpenDomains = (openDomains: Record<string, boolean>) => setPlanNewOpenDomainsByChild((items) => ({ ...items, [String(selectedChildId)]: openDomains }));
  const savePlanNewData = async (openDomains: Record<string, boolean>) => {
    const nextOpenDomainsByChild = { ...planNewOpenDomainsByChild, [String(selectedChildId)]: openDomains };
    setPlanNewOpenDomainsByChild(nextOpenDomainsByChild);
    await saveCloudData({ ...cloudExtras, children, goals, evaluationPeriods: currentEvaluationPeriods, evaluationPeriodsByChild, domains, domainIcons, collapsedGoalIds, planNewOpenDomainsByChild: nextOpenDomainsByChild, planNewOpenDomains: openDomains });
  };
  ...
  const editDomain = (domain: string) => {
    ...
    const fallback: Goal = { id: 0, childId: selectedChildId, domain, longTerm: "", shortTerm: [], from: "01/07/2026", to: "30/08/2026", statuses: currentEvaluationPeriods.map(() => "") };
  ...
  const editDomainSetting = (domain: string) => {
    openGoalDialog({ mode: "edit-domain-setting", initialDomain: domain, initialIcon: domainIcons[domain] });
  };
  const deleteDomain = (domain: string) => {
    if (domains.length <= 1) { window.alert("Cần giữ lại ít nhất một lĩnh vực."); return; }
    if (!window.confirm(`Xóa lĩnh vực “${domain}” và các mục tiêu thuộc lĩnh vực này?`)) return;
    setDomains((items) => items.filter((item) => item !== domain));
    setDomainIcons((items) => { const next = { ...items }; delete next[domain]; return next; });
    setGoals((items) => items.filter((goal) => goal.domain !== domain));
    setPlanNewHiddenDomainsByChild((items) => Object.fromEntries(Object.entries(items).map(([childId, values]) => [childId, values.filter((item) => item !== domain)])));
    setPlanNewOpenDomainsByChild((items) => Object.fromEntries(Object.entries(items).map(([childId, values]) => { const next = { ...values }; delete next[domain]; return [childId, next]; })));
  };
  const hideDomainInPlanNew = (domain: string) => {
    if (!window.confirm(`Xóa lĩnh vực “${domain}” và toàn bộ mục tiêu của trẻ này? Dữ liệu sẽ bị xóa hoàn toàn.`)) return;
    setGoals((items) => items.filter((goal) => !(goal.childId === selectedChildId && goal.domain === domain)));
    setPlanNewHiddenDomainsByChild((items) => {
      const childKey = String(selectedChildId);
      const next = { ...items };
      next[childKey] = (next[childKey] ?? []).filter((item) => item !== domain);
      return next;
    });
    setPlanNewOpenDomainsByChild((items) => {
      const childKey = String(selectedChildId);
      const next = { ...items };
      if (next[childKey]) {
        const childOpenDomains = { ...next[childKey] };
        delete childOpenDomains[domain];
        next[childKey] = childOpenDomains;
      }
      return next;
    });
  };
  const saveGoalDialog = ({ mode, text, domain, goalId, periodLabel, periodLabels, domainIcon, shortTerm, statuses, note }: GoalDialogSave) => {
    if (mode === "domain") {
      setPlanNewHiddenDomainsByChild((items) => {
        ...
        if (!current.includes(domain)) return items;
        return { ...items, [childKey]: current.filter((item) => item !== domain) };
      });
      ...
      setGoals((items) => [...items, { id: Math.max(0, ...items.map((item) => item.id)) + 1, childId: selectedChildId, domain, longTerm: text, shortTerm: [], from: "01/07/2026", to: "30/08/2026", statuses: currentEvaluationPeriods.map(() => "") }]);
      setPlanNewOpenDomainsByChild((items) => ({ ...items, [String(selectedChildId)]: { ...(items[String(selectedChildId)] ?? {}), [domain]: true } }));
      ...
      setDomainIcons((items) => {
        const next = { ...items, [domain]: domainIcon ?? items[previousDomain] ?? DOMAIN_ICON_OPTIONS[0].value };
        if (previousDomain !== domain) delete next[previousDomain];
        return next;
      });
      ...
      setEvaluationPeriodsByChild((items) => items[String(selectedChildId)] ? items : { ...items, [String(selectedChildId)]: [...currentEvaluationPeriods] });
      setGoals((items) => [...items, { id: Math.max(0, ...items.map((item) => item.id)) + 1, childId: selectedChildId, domain: domain || domains[0] || DOMAIN_OPTIONS[0], longTerm: text, shortTerm: [], from: "01/07/2026", to: "30/08/2026", statuses: currentEvaluationPeriods.map(() => "") }]);
      ...
      setGoals((items) => items.map((goal) => {
        const isTargetGoal = goal.childId === childId && goal.domain === previousDomain;
        if (!isTargetGoal) return goal;
        return { ...goal, domain, longTerm: text, shortTerm: shortTerm ?? goal.shortTerm, statuses: normalizedLabels.map((_, index) => statuses?.[index] ?? goal.statuses[index] ?? ""), note };
      }));
      ...
      setGoals((items) => items.map((goal) => goal.childId === selectedChildId ? { ...goal, statuses: [...goal.statuses, ""] } : goal));
    }
    setGoalDialog(null);
  };
  ...
  const exportChildPdf = (child: Child) => openSharePrintWindow(child, goals.filter((goal) => goal.childId === child.id), periodsForChild(child.id), domainIcons);
  const exportChildWord = (child: Child) => downloadShareWord(child, goals.filter((goal) => goal.childId === child.id), periodsForChild(child.id), domainIcons);
  settingsChildList = children;
  settingsSelectedChildId = selectedChildId;
  settingsExportPdfHandler = exportChildPdf;
  settingsExportWordHandler = exportChildWord;
  if (shareChildSlug !== null) {
    ...
    return <ShareView child={sharedChild} goals={goals.filter((goal) => goal.childId === sharedChild.id)} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} />;
  }
  if (view === "plan-new") {
    return <div className="app-shell"><SidebarV2 active={currentView} onChange={navigate} /><main className="main-content"><PlanNewView key={`plan-new-${selectedChildId}`} childList={children} selectedChildId={selectedChildId} onSelectChild={setSelectedChildId} goals={goals} hiddenDomains={planNewHiddenDomains} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} initialOpenDomains={planNewOpenDomainsByChild[String(selectedChildId)] ?? {}} onOpenDomainsChange={updatePlanNewOpenDomains} onSave={savePlanNewData} onAddDomain={() => openGoalDialog({ mode: "domain" })} onAddLong={(domain) => openGoalDialog({ mode: "long", initialDomain: domain ?? currentChildGoals[0]?.domain })} onAddShort={(goal) => openGoalDialog({ mode: "short", initialGoalId: goal.id })} onAddPeriod={() => openGoalDialog({ mode: "period" })} onEditDomain={(goal) => openGoalDialog({ mode: "edit-domain", goalId: goal.id, editingGoal: goal, initialDomain: goal.domain, initialIcon: domainIcons[goal.domain] })} onEditLong={(goal) => openGoalDialog({ mode: "edit-long", goalId: goal.id, initialValue: goal.longTerm })} onDeleteGoal={deleteGoal} onEditShort={(goal, index) => openGoalDialog({ mode: "edit-short", goalId: goal.id, shortIndex: index, initialValue: goal.shortTerm[index] })} onDeleteShort={deleteShort} onEditPeriod={(goal, index) => openGoalDialog({ mode: "edit-period", goalId: goal.id, periodIndex: index, initialValue: goal.statuses[index], initialPeriodLabel: evaluationPeriods[index] })} onDeleteDomain={hideDomainInPlanNew} onStatusChange={updateStatus} onNoteClick={setNoteGoalId} /></main>{showChildForm && <ChildForm child={editingChild} onCancel={() => { setShowChildForm(false); setEditingChild(undefined); }} onSave={saveChild} />}{goalDialog && <GoalDialog key={`${goalDialog.mode}-${goalDialog.goalId ?? "new"}-${goalDialog.shortIndex ?? ""}-${goalDialog.initialDomain ?? ""}`} mode={goalDialog.mode} goals={currentChildGoals} domains={domains} evaluationPeriods={evaluationPeriods} editingGoal={goalDialog.editingGoal} initialValue={goalDialog.initialValue} initialGoalId={goalDialog.goalId} initialDomain={goalDialog.initialDomain ?? currentChildGoals[0]?.domain} initialPeriodLabel={goalDialog.initialPeriodLabel} initialIcon={goalDialog.initialIcon} onCancel={() => setGoalDialog(null)} onSave={saveGoalDialog} />}{noteGoalId !== null && <NoteDialog key={noteGoalId} goal={noteGoal} onCancel={() => setNoteGoalId(null)} onSave={saveNote} />}</div>;
  }
  return <div className="app-shell"><SidebarV2 active={currentView} onChange={navigate} /><main className="main-content">{view === "overview" && <OverviewViewV2 childList={children} selectedChildId={selectedChildId} onSelectChild={setSelectedChildId} goals={goals} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} onOpenPlan={() => setView("plan")} />}{view === "plan" && <PlanViewV2 childList={children} selectedChildId={selectedChildId} onSelectChild={setSelectedChildId} goals={goals} evaluationPeriods={evaluationPeriods} domainIcons={domainIcons} collapsedGoalIds={collapsedGoalIds} onToggleCollapse={toggleCollapse} onStatusChange={updateStatus} onNoteClick={setNoteGoalId} onAddDomain={() => openGoalDialog({ mode: "domain" })} onAddLong={() => openGoalDialog({ mode: "long", initialDomain: currentChildGoals[0]?.domain })} onAddShort={(goal) => { const targetGoal = goal ?? currentChildGoals[0]; if (!targetGoal) { window.alert("Hãy thêm mục tiêu dài hạn trước."); return; } openGoalDialog({ mode: "short", initialGoalId: targetGoal.id }); }} onAddPeriod={() => openGoalDialog({ mode: "period" })} onEditDomain={(goal) => openGoalDialog({ mode: "edit-domain", goalId: goal.id, editingGoal: goal, initialDomain: goal.domain, initialIcon: domainIcons[goal.domain] })} onEditLong={(goal) => openGoalDialog({ mode: "edit-long", goalId: goal.id, initialValue: goal.longTerm })} onDeleteGoal={deleteGoal} onEditShort={(goal, index) => openGoalDialog({ mode: "edit-short", goalId: goal.id, shortIndex: index, initialValue: goal.shortTerm[index] })} onDeleteShort={deleteShort} onEditPeriod={(goal, index) => openGoalDialog({ mode: "edit-period", goalId: goal.id, periodIndex: index, initialValue: goal.statuses[index], initialPeriodLabel: evaluationPeriods[index] })} />}{view === "children" && <ChildrenView childList={children} onAdd={() => { setEditingChild(undefined); setShowChildForm(true); }} onEdit={(child) => { setEditingChild(child); setShowChildForm(true); }} onDelete={deleteChild} onSelectPlan={(id) => { setSelectedChildId(id); setView("plan"); }} />}{view === "objective-form" && <ObjectiveForm childList={children} onCancel={() => setView("plan")} onSaved={saveGoal} />}{view === "settings" && <SettingsView darkMode={darkMode} onToggleTheme={() => setDarkMode((value) => !value)} domains={domains} domainIcons={domainIcons} onAddDomain={addDomain} onEditDomain={editDomainSetting} onDeleteDomain={deleteDomain} />}</main>{showChildForm && <ChildForm child={editingChild} onCancel={() => { setShowChildForm(false); setEditingChild(undefined); }} onSave={saveChild} />}{goalDialog && <GoalDialog mode={goalDialog.mode} goals={currentChildGoals} domains={domains} evaluationPeriods={evaluationPeriods} editingGoal={goalDialog.editingGoal} initialValue={goalDialog.initialValue} initialGoalId={goalDialog.goalId} initialDomain={goalDialog.initialDomain ?? currentChildGoals[0]?.domain} initialPeriodLabel={goalDialog.initialPeriodLabel} initialIcon={goalDialog.initialIcon} onCancel={() => setGoalDialog(null)} onSave={saveGoalDialog} />}{noteGoalId !== null && <NoteDialog goal={noteGoal} onCancel={() => setNoteGoalId(null)} onSave={saveNote} />}</div>;
}
