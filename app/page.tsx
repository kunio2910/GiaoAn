...
function GoalDialog({ mode, goals, domains, evaluationPeriods, editingGoal, initialValue = "", initialGoalId, initialDomain, initialPeriodLabel = "", initialIcon, onCancel, onSave }: { mode: GoalDialogMode; goals: Goal[]; domains: string[]; evaluationPeriods: string[]; editingGoal?: Goal; initialValue?: string; initialGoalId?: number; initialDomain?: string; initialPeriodLabel?: string; initialIcon?: DomainIcon; onCancel: () => void; onSave: (data: GoalDialogSave) => void }) {
  ...
  const canSave = isSettingsDomainEdit ? Boolean(domainName.trim()) : isDomainEdit ? Boolean(domainName.trim()) && periodLabels.length > 0 && periodLabels.every((label) => Boolean(label.trim())) : isPeriodEdit ? Boolean(periodLabel.trim()) : mode === "domain" ? Boolean(domain) : Boolean(text.trim());
...
export default function Home() {
  ...
  const saveGoalDialog = ({ mode, text, domain, goalId, periodLabel, periodLabels, domainIcon, shortTerm, statuses, note }: GoalDialogSave) => {
    if (mode === "domain") {
      ...
      if (new Set(normalizedLabels.map((label) => label.toLocaleLowerCase())).size !== normalizedLabels.length) { window.alert("Tên thời gian theo tuần không được trùng nhau."); return; }
      setGoals((items) => items.map((goal) => goal.childId === selectedChildId && goal.domain === previousDomain ? { ...goal, domain, longTerm: text, shortTerm: shortTerm ?? goal.shortTerm, statuses: normalizedLabels.map((_, index) => statuses?.[index] ?? goal.statuses[index] ?? "Chưa đạt"), note } : goal));
