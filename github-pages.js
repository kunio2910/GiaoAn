(function () {
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (mode === 'domain') {
      state.goals.push({ id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId: state.selectedChildId, domain: selectedDomain, longTerm: text, shortTerm: [], from: '01/07/2026', to: '30/08/2026', statuses: periodsForChild(state.selectedChildId).map(() => '') });
      ...
      state.goals.push({ id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId: state.selectedChildId, domain: goalDomain, longTerm: text, shortTerm: [], from: '01/07/2026', to: '30/08/2026', statuses: periodsForChild(state.selectedChildId).map(() => '') });
      const hiddenDomains = hiddenPlanNewDomainsForChild(state.selectedChildId);
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (event.target.id === 'objective-form') { event.preventDefault(); const shortTerm = draftShortGoals.map((item) => item.trim()).filter(Boolean); if (!shortTerm.length || !$('#objective-long').value.trim()) return; const childId = Number($('#objective-child').value); const goal = { id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId, domain: $('#objective-domain').value, longTerm: $('#objective-long').value.trim(), shortTerm, from: $('#objective-from').value, to: $('#objective-to').value, statuses: periodsForChild(childId).map(() => '') }; state.goals.push(goal); state.selectedChildId = goal.childId; persist(); navigate('plan'); }
  });
