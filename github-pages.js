(function () {
  ...
  const statusMarkup = (status, goalId, periodIndex, readOnly) => {
    if (readOnly && !status) return '';
    const tone = status === 'Đạt' ? 'achieved' : status === 'Manh nha' ? 'emerging' : status ? 'not-achieved' : 'pending';
    const options = statuses.map((item) => `<option value="${esc(item)}" ${item === status ? 'selected' : ''}>${item || 'Chưa đánh giá'}</option>`).join('');
    return readOnly ? `<div class="status-readonly ${tone}"><span class="status-dot"></span><span>${esc(status)}</span></div>` : `<label class="status-select ${tone}">${status ? '<span class="status-dot"></span>' : ''}<select data-goal-id="${goalId}" data-week="${periodIndex}">${options}</select>${icon('chevron')}</label>`;
  };
  function renderGoalsBoard(goals, childId = state.selectedChildId) {
    ...
    const cards = goals.map((goal) => {
      ...
      return `<article class="goal-card ${standalone ? 'standalone-goal-card' : ''}" data-goal-card data-search="${searchText}"><header class="goal-card-header"><div class="goal-domain-heading"><span class="goal-domain-icon">${icon(domainIconOption(state.domainIcons?.[goal.domain] || defaultDomainIcons[goal.domain]).value)}</span><div><span class="goal-card-eyebrow">Lĩnh vực</span><h3>${esc(goal.domain || 'Chưa phân loại')}</h3></div></div><div class="goal-card-actions"><button type="button" class="goal-card-action edit" data-action="edit-long" data-goal-id="${goal.id}">${icon('edit')}Sửa</button><button type="button" class="goal-card-action delete" data-action="delete-goal" data-goal-id="${goal.id}">${icon('trash')}Xóa</button><button type="button" class="goal-card-note-button" data-action="open-note" data-goal-id="${goal.id}" aria-label="Ghi chú mục tiêu">${icon('note')}</button></div></header><div class="goal-card-main ${standalone ? 'standalone-goal-main' : ''}"><section class="goal-long-section"><div class="goal-card-section-head"><h4>${standalone ? 'Mục tiêu' : 'Mục tiêu dài hạn'}</h4>${standalone ? '' : `<button type="button" class="outline-button compact goal-long-add-button" data-action="add-long" aria-label="Thêm mục tiêu">${icon('plus')}Thêm</button>`}</div><p>${esc(goal.longTerm || 'Chưa nhập mục tiêu')}</p><button type="button" class="text-action" data-action="edit-long" data-goal-id="${goal.id}">${icon('edit')}Chỉnh sửa ${standalone ? 'mục tiêu' : 'mục tiêu dài hạn'}</button></section>${standalone ? '' : `<section class="goal-short-section"><div class="goal-card-section-head"><h4>Mục tiêu ngắn hạn</h4><button type="button" class="outline-button compact" data-action="add-short" data-goal-id="${goal.id}">${icon('plus')}Thêm</button></div><div class="short-goal-list">${shortGoals}</div></section>`}</div><section class="goal-results-section"><div class="goal-card-section-head"><div><h4>Kết quả theo tuần</h4><p>Cập nhật trạng thái trực tiếp theo từng giai đoạn.</p></div><button type="button" class="outline-button compact" data-action="add-period">${icon('plus')}Thêm thời gian</button></div><div class="goal-period-grid">${periods.map((label, periodIndex) => `<div class="goal-period"><div class="goal-period-label-row"><span class="goal-period-label">${esc(label)}</span><span class="period-actions"><button type="button" class="period-action" data-action="edit-period" data-goal-id="${goal.id}" data-period-index="${periodIndex}" aria-label="Sửa kết quả ${esc(label)}">${icon('edit')}</button><button type="button" class="period-action delete" data-action="delete-period" data-period-index="${periodIndex}" aria-label="Xóa mốc ${esc(label)}">${icon('trash')}</button></span></div>${statusMarkup(goal.statuses?.[periodIndex] ?? '', goal.id, periodIndex, false)}</div>`).join('')}</div></section><footer class="goal-card-footer"><div><h4>Ghi chú</h4><p>${goal.note ? esc(goal.note) : '<span class="cell-placeholder">Chưa có ghi chú.</span>'}</p></div><button type="button" class="note-edit-button" data-action="open-note" data-goal-id="${goal.id}">${icon('note')}${goal.note ? 'Chỉnh sửa ghi chú' : 'Thêm ghi chú'}</button></footer></article>`;
    }).join('');
  ...
  function renderGoalsTable(goals, readOnly = false, childId = state.selectedChildId) {
    ...
    return renderReadOnlyGoalsTables(goals, childId);
  ...
  function exportIconSvg(name) {
    ...
    return `<svg class="export-domain-svg" style="width:23px;height:23px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round" viewBox="0 0 24 24" aria-hidden="true">${currentPaths[name] || currentPaths.target}</svg>`;
    /*
    const paths = { overview: '<rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/>', plan: '<path d="M4 5.5A2.5 2.5 0 0 1 6.5 3H20v16H6.5A2.5 2.5 0 0 0 4 21.5z"/><path d="M4 5.5v16"/><path d="M8 7h8M8 11h8M8 15h5"/>', children: '<circle cx="9" cy="8" r="3"/><path d="M3.5 20a5.5 5.5 0 0 1 11 0"/><circle cx="17.5" cy="10" r="2.2"/><path d="M15.5 16.8a4.2 4.2 0 0 1 5 3.2"/>', target: '<circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="4"/><path d="m16.5 7.5 3-3M19.5 4.5v3h-3"/>', group: '<circle cx="9" cy="8" r="3"/><path d="M3.5 20a5.5 5.5 0 0 1 11 0"/><circle cx="17.5" cy="9" r="2.2"/><path d="M15.5 17a4.2 4.2 0 0 1 5 3"/>', eye: '<path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6z"/><circle cx="12" cy="12" r="2.5"/>', note: '<path d="M4.5 5h15v11H10l-4.5 4V5z"/><path d="M8 9h8M8 12h5"/>', calendar: '<rect x="3.5" y="5" width="17" height="15" rx="2"/><path d="M7 3v4M17 3v4M3.5 9h17"/>', heart: '<path d="M20.8 8.7c0 5.2-8.8 10.1-8.8 10.1S3.2 13.9 3.2 8.7A4.4 4.4 0 0 1 12 6.9a4.4 4.4 0 0 1 8.8 1.8Z"/>', star: '<path d="m12 3 2.8 5.7 6.2.9-4.5 4.4 1.1 6.2-5.6-2.9-5.6 2.9 1.1-6.2L3 9.6l6.2-.9z"/>', puzzle: '<path d="M8.5 4H12v3.5a2.5 2.5 0 1 0 5 0V4h3v4.5h-3a2.5 2.5 0 1 1 0 5h3V20h-4.5v-3a2.5 2.5 0 1 0-5 0v3H4v-4.5h3a2.5 2.5 0 1 1 0-5H4V7.5h4.5z"/>', music: '<path d="M9 18V5l10-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="16" cy="16" r="3"/>', brain: '<path d="M9.5 4.5A3.5 3.5 0 0 0 6 8v.5a3 3 0 0 0-1 5.7A3.5 3.5 0 0 0 8.5 19H10V6.5a2 2 0 0 0-.5-2Z"/><path d="M14.5 4.5A3.5 3.5 0 0 1 18 8v.5a3 3.5 0 0 1 1 5.7 3.5 3.5 0 0 1-3.5 4.8H14V6.5a2 2 0 0 1 .5-2Z"/><path d="M10 9H8m2 4H7m7-4h2m-2 4h3"/>', hand: '<path d="M6.5 11.5V6.8a1.3 1.3 0 0 1 2.6 0v4.1V5.2a1.3 1.3 0 0 1 2.6 0v5.7V4.8a1.3 1.3 0 0 1 2.6 0v6.1V6.2a1.3 1.3 0 0 1 2.6 0v7.2c0 4.2-2.1 6.6-6 6.6-2.8 0-4.5-1.4-5.5-3.4L4 13.8a1.6 1.6 0 0 1 2.5-2.3Z"/>" };
    return `<svg class="export-domain-svg" viewBox="0 0 24 24" aria-hidden="true">${paths[name] || paths.target}</svg>`;
    */
  }
  function shareExportHtml(child, goals) {
    ...
    const goalGroups = goals.reduce((groups, goal) => { const group = groups.find((item) => item.domain === goal.domain); if (group) group.goals.push(goal); else groups.push({ domain: goal.domain, goals: [goal] }); return groups; }, []);
    const rows = goals.length ? goalGroups.map(({ domain, goals: domainGoals }) => domainGoals.map((goal, goalIndex) => `<tr>${goalIndex === 0 ? `<td class="export-domain-cell" rowspan="${domainGoals.length}"><span class="export-domain-icon">${exportIconSvg(domainIconOption(state.domainIcons?.[domain] || defaultDomainIcons[domain]).value)}</span><strong>${esc(domain)}</strong></td>` : ''}<td class="export-long-term-cell">${esc(goal.longTerm || 'Chưa nhập mục tiêu')}</td><td class="export-short-term-cell"><ul>${(goal.shortTerm?.length ? goal.shortTerm : ['Chưa có mục tiêu ngắn hạn']).map((item) => `<li>${esc(item)}</li>`).join('')}</ul></td>${periods.map((_, periodIndex) => { const status = goal.statuses?.[periodIndex] ?? ''; return `<td class="export-result-cell"><span class="export-status ${statusClass(status)}"><i></i>${esc(status)}</span></td>`; }).join('')}<td class="export-note-cell">${esc(goal.note || 'Chưa có ghi chú')}</td></tr>`).join('')).join('') : `<tr><td colspan="${periods.length + 4}" class="export-empty">Chưa có mục tiêu phát triển</td></tr>`;
  ...
  function normalizeExportSummaryHtml(html, child) {
    const info = getPlanInfo(child);
    const summaryTable = `<table class="summary-table"><tbody><tr><td class="summary-main-cell"><strong>${esc(child.name)}</strong><span>Ngày sinh: ${esc(child.birthday)}</span><span>Tuổi thực: ${esc(calculateAge(child.birthday))}</span></td><td class="summary-meta-cell"><span>Người lập kế hoạch: ${esc(info.planner)}</span><span>Ngày lập kế hoạch: ${esc(info.planDate)}</span></td><td class="evaluation-cell"><strong>Thông tin lượng giá</strong><span>Ngày lượng giá:</span><b>${esc(info.evaluationDates)}</b></td></tr></tbody></table>`;
  ...
  function openExportChildModal(type) {
    if (!state.children.length) { window.alert('Hãy thêm hồ sơ trẻ trước khi xuất file.'); return; }
    let modal = $('#export-child-modal');
    if (!modal) {
      document.body.insertAdjacentHTML('beforeend', `<div class="modal-backdrop" id="export-child-modal" hidden><div class="modal-card export-child-modal" role="dialog" aria-modal="true" aria-labelledby="export-child-title"><div class="modal-head"><div><span class="modal-kicker">XUẤT HỒ SƠ CHIA SẺ</span><h2 id="export-child-title"></h2><p>Chọn đúng hồ sơ để xuất dữ liệu riêng của trẻ.</p></div><button class="close-button" data-action="close-export-child" type="button" aria-label="Đóng">×</button></div><label class="field"><span>Hồ sơ trẻ</span><div class="select-wrap"><select id="export-child-select"></select>${icon('chevron')}</div></label><p class="modal-description">Nội dung sẽ bám sát màn hình share, giữ nguyên icon lĩnh vực và dùng khổ giấy ngang.</p><div class="modal-actions"><button class="button" data-action="close-export-child" type="button">Hủy</button><button class="button primary" data-action="confirm-export-child" type="button">Xuất file</button></div></div></div>`);
      modal = $('#export-child-modal');
    }
    $('#export-child-title').textContent = `Chọn trẻ để xuất ${type === 'pdf' ? 'PDF' : 'Word'}`;
    modal.dataset.exportType = type;
    $('#export-child-select').innerHTML = state.children.map((child) => `<option value="${child.id}" ${child.id === state.selectedChildId ? 'selected' : ''}>${esc(child.name)}</option>`).join('');
    modal.removeAttribute('hidden');
  }
  function closeExportChildModal() { $('#export-child-modal')?.setAttribute('hidden', ''); }
  function confirmExportChild() {
    const modal = $('#export-child-modal');
    const child = childById(Number($('#export-child-select')?.value));
    if (!modal || !child) return;
    const type = modal.dataset.exportType;
    closeExportChildModal();
    if (type === 'pdf') exportSharePdf(child); else exportShareWord(child);
  }
  function exportSharePdf(childOverride) { const child = childOverride || selectedChild(); if (!child) { window.alert('Hãy thêm hoặc chọn một hồ sơ trẻ trước khi xuất file.'); return; } const printWindow = window.open('', '_blank'); if (!printWindow) { window.alert('Trình duyệt đang chặn cửa sổ xuất file. Hãy cho phép pop-up rồi thử lại.'); return; } printWindow.document.open(); printWindow.document.write(normalizeExportSummaryHtml(shareExportHtml(child, state.goals.filter((goal) => goal.childId === child.id)), child)); printWindow.document.close(); printWindow.addEventListener('load', () => { printWindow.focus(); printWindow.print(); }); }
  function exportShareWord(childOverride) { const child = childOverride || selectedChild(); if (!child) { window.alert('Hãy thêm hoặc chọn một hồ sơ trẻ trước khi xuất file.'); return; } const blob = new Blob([`\ufeff${normalizeExportSummaryHtml(shareExportHtml(child, state.goals.filter((goal) => goal.childId === child.id)), child)}`], { type: 'application/msword' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `Ke-hoach-giao-duc-${childNameSlug(child.name) || 'tre'}.doc`; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(link.href), 1000); }
  ...
  function openGoalModal(mode, goalId, shortIndex, domainNameOverride = '') {
    ...
    const titles = { domain: 'Thêm lĩnh vực', long: 'Thêm mục tiêu', short: 'Thêm mục tiêu ngắn hạn', 'edit-domain': 'Chỉnh sửa lĩnh vực', 'edit-domain-setting': 'Chỉnh sửa lĩnh vực', 'edit-long': 'Chỉnh sửa mục tiêu dài hạn', 'edit-short': 'Chỉnh sửa mục tiêu ngắn hạn', period: 'Thêm thời gian kết quả' };
    const labels = { domain: 'Tên lĩnh vực', long: 'Tên mục tiêu', short: 'Mục tiêu ngắn hạn', 'edit-long': 'Mục tiêu dài hạn', 'edit-short': 'Mục tiêu ngắn hạn', period: 'Tên thời gian đánh giá' };
    ...
    $('#goal-dialog-label').textContent = mode === 'edit-long' && goal?.goalType === 'goal' ? 'Mục tiêu' : labels[mode] || 'Nội dung';
    ...
    $('#goal-target-type-field').hidden = mode !== 'long';
    $('#goal-dialog-target-type').value = goal?.goalType === 'goal' ? 'goal' : 'long-term';
    ...
    $('#goal-dialog-status').innerHTML = statuses.map((item) => `<option value="${esc(item)}">${item || 'Chưa đánh giá'}</option>`).join('');
    $('#goal-dialog-status').value = isPeriodEdit && goal ? goal.statuses?.[shortIndex] ?? '' : '';
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action) {
      ...
      if (action.dataset.action === 'export-share-pdf') openExportChildModal('pdf');
      if (action.dataset.action === 'export-share-word') openExportChildModal('word');
      if (action.dataset.action === 'close-export-child') closeExportChildModal();
      if (action.dataset.action === 'confirm-export-child') confirmExportChild();
  ...
  document.addEventListener('submit', (event) => {
    ...
    const targetType = $('#goal-dialog-target-type')?.value || 'long-term';
    ...
    if (mode === 'domain') {
      state.goals.push({ id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId: state.selectedChildId, domain: selectedDomain, goalType: 'long-term', longTerm: text, shortTerm: [], from: '01/07/2026', to: '30/08/2026', statuses: periodsForChild(state.selectedChildId).map(() => '') });
      ...
      planNewOpenDomains[selectedDomain] = true;
      state.planNewOpenDomains = planNewOpenDomains;
      state.planNewOpenDomainsByChild = { ...(state.planNewOpenDomainsByChild || {}), [String(state.selectedChildId)]: { ...planNewOpenDomains } };
      ...
      state.goals.forEach((item) => { if (item.childId === childId && item.domain === previousDomain) { item.domain = domainName; item.longTerm = text; item.shortTerm = shortTerm; item.statuses = nextPeriodLabels.map((_, index) => nextStatuses[index] ?? item.statuses?.[index] ?? ''); item.note = nextNote; } });
      state.domains = state.domains.map((item) => item === previousDomain ? domainName : item);
      domains = state.domains;
      if (previousDomain !== domainName) delete state.domainIcons[previousDomain];
      ...
      state.goals.push({ id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId: state.selectedChildId, domain: goalDomain, goalType: targetType === 'goal' ? 'goal' : 'long-term', longTerm: text, shortTerm: [], from: '01/07/2026', to: '30/08/2026', statuses: periodsForChild(state.selectedChildId).map(() => '') });
      const hiddenDomains = hiddenPlanNewDomainsForChild(state.selectedChildId);
      ...
      setPeriodsForChild(state.selectedChildId, nextPeriods);
      state.goals.filter((goal) => goal.childId === state.selectedChildId).forEach((goal) => goal.statuses.push(''));
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (event.target.id === 'objective-form') { event.preventDefault(); const shortTerm = draftShortGoals.map((item) => item.trim()).filter(Boolean); if (!shortTerm.length || !$('#objective-long').value.trim()) return; const childId = Number($('#objective-child').value); const goal = { id: Math.max(0, ...state.goals.map((item) => item.id)) + 1, childId, domain: $('#objective-domain').value, goalType: 'long-term', longTerm: $('#objective-long').value.trim(), shortTerm, from: $('#objective-from').value, to: $('#objective-to').value, statuses: periodsForChild(childId).map(() => '') }; state.goals.push(goal); state.selectedChildId = goal.childId; persist(); navigate('plan'); }
  });
